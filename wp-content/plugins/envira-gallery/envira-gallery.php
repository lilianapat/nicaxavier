...
if ( ! class_exists( 'Envira_Gallery' ) ) :
	...
	final class Envira_Gallery {
		...
		public function require_global() {
			...
			require_once trailingslashit( ENVIRA_DIR ) . 'src/Functions/ai-image-feature.php';
		}
		...
		public static function activate( $network_wide ) {
			...
			if ( is_multisite() && $network_wide ) {
				...
				// Set default license option.
				$option = get_option( 'envira_gallery' );
				if ( empty( get_option( 'envira_lite_version' ) ) && empty( $option ) ) {
					// Add transient to trigger redirect.
					set_transient( '_envira_activation_redirect', 1, 120 );
				}
				if ( ! $option || empty( $option ) ) {
					update_option( 'envira_gallery', self::default_options() );
				}
			...
			if ( empty( $over_time['installed_pro'] ) ) {
				$over_time['installed_version'] = ENVIRA_VERSION;
		...
		public function redirect_to_wizard() {
			if ( ! get_transient( '_envira_activation_redirect' ) ) {
				return;
			}

			delete_transient( '_envira_activation_redirect' );
			...
			$envira_display_welcome = get_option( 'envira_wizard' );

			if ( ! $envira_display_welcome ) {
				update_option( 'envira_wizard', 'yes' );
