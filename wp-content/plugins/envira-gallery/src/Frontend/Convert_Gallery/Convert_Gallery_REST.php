...
namespace Envira\Frontend\Convert_Gallery;
 ...
 * Convert Gallery REST Class.
 */
class Convert_Gallery_REST {
	use Convert_Gallery_Trait;
	...
	public function verify_rest_nonce( $request ) {
		...
		if ( ! wp_verify_nonce( $nonce, 'wp_rest' ) ) {
			return new WP_REST_Response( [ 'message' => __( 'Security check failed. You are not authorized. Please refresh and try again.', 'envira-gallery' ) ], 403 );
		}
	...
	public function envira_convert_wordpress_gallery( $request ) {
		...
		if ( empty( $sanitized_images ) || ! is_array( $sanitized_images ) ) {
			return new WP_REST_Response(
				[
					'message' => __( 'No images provided. Please add at least one image to continue.', 'envira-gallery' ),
				],
				400
			);
		}
		...
		foreach ( $sanitized_images as $image ) {
			if ( empty( $image['id'] ) || empty( $image['url'] ) ) {
				return new WP_REST_Response(
					[
						'message' => __( 'Each image must have an ID and a URL. Please check your images and try again.', 'envira-gallery' ),
					],
					400
				);
			}
	...
	public function envira_convert_bulk_galleries( $request ) {
		...
		if ( empty( $selected_posttype ) ) {
			return new WP_REST_Response( [ 'message' => __( 'A post type is required for conversion. Please make a selection.', 'envira-gallery' ) ], 400 );
		}

		if ( ! post_type_exists( $selected_posttype ) ) {
			return new WP_REST_Response( [ 'message' => __( 'Post type not recognized. Please check your selection and try again.', 'envira-gallery' ) ], 404 );
		}
		...
		if ( empty( $posts ) ) {
			return new WP_REST_Response( [ 'message' => __( 'No items found for the selected post type. Please try selecting a different option.', 'envira-gallery' ) ], 400 );
		}
		...
		if ( empty( $found_posts ) ) {
			return new WP_REST_Response( [ 'message' => __( 'No galleries were found in the selected post type.', 'envira-gallery' ) ], 400 );
		}
	...
	public function envira_convert_process_items( $request ) {
		...
		if ( empty( $post_id ) ) {
			return new WP_REST_Response( [ 'error' => __( 'A valid post ID is required.', 'envira-gallery' ) ], 400 );
		}
		...
		if ( ! $post ) {
			return new WP_REST_Response( [ 'error' => __( 'Post not found.', 'envira-gallery' ) ], 400 );
		}
		...
		if ( $needs_update ) {
			...
			return new WP_REST_Response( [ 'success' => __( 'Galleries converted to Envira Galleries.', 'envira-gallery' ) ], 200 );
		} else {
			return new WP_REST_Response(
				[
					'error'    => __( 'This post contains gallery content that is not compatible with the conversion process.', 'envira-gallery' ),
