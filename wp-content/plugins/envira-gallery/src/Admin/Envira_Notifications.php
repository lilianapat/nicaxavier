...
class Envira_Notifications {
	...
	public function get_notifications_with_human_readeable_start_time( $notifications ) {
		...
		foreach ( $notifications as $key => $notification ) {
			...
			$modified_start_time            = sprintf( __( '%1$s ago', 'envira-gallery' ), human_time_diff( strtotime( $notification['start'] ), time() ) );
		...
		<li>
			...
			<div class="envira-notification-content">
				...
				<div class="envira-notification-actions">
					...
					?>
					<a href="#" class="envira-button-text envira-notification-dismiss" data-id="<?php echo esc_attr( $notification['id'] ); ?>"><?php esc_html_e( 'Dismiss', 'envira-gallery' ); ?></a>
				</div>
			</div>
		</li>
		<?php
