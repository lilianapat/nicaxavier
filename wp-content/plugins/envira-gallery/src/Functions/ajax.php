...
function envira_gallery_change_image_status() {
	...
	// Prepare variables.
	$post_id      = isset( $_POST['post_id'] ) ? absint( wp_unslash( $_POST['post_id'] ) ) : null;
	$attach_id    = isset( $_POST['gallery_id'] ) ? sanitize_key( $_POST['gallery_id'] ) : null;
...
function envira_gallery_ajax_change_preview() {
	...
	if ( isset( $_POST['data'] ) ) {
		...
		// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Sanitized in loop.
		foreach ( wp_unslash( $_POST['data'] ) as $index => $field ) {

			// Skip if this isn't a configuration field.
...
function envira_gallery_ajax_insert_images() {
	...
	if ( isset( $_POST['images'] ) ) {
		// Sanitize images and decode JSON.
		$images = json_decode( wp_unslash( $_POST['images'] ), true ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	}
	...
	// If no gallery has been set, set it now.
	...
	foreach ( (array) $images as $i => $image ) {
		...
		if ( array_key_exists( $image['id'], $gallery_data['gallery'] ) ) {
			continue;
		}
...
function envira_gallery_ajax_sort_images() {
	...
	// Prepare variables.
	$order        = isset( $_POST['order'] ) ? explode( ',', sanitize_text_field( wp_unslash( $_POST['order'] ) ) ) : '';
	$post_id      = isset( $_POST['post_id'] ) ? absint( wp_unslash( $_POST['post_id'] ) ) : null;
	$gallery_data = get_post_meta( $post_id, '_eg_gallery_data', true );
...
function envira_gallery_ajax_remove_images() {
	...
	// Prepare variables.
	$post_id      = isset( $_POST['post_id'] ) ? absint( wp_unslash( $_POST['post_id'] ) ) : null;
	$attach_ids   = isset( $_POST['attachment_ids'] ) ? array_map( 'sanitize_text_field', wp_unslash( $_POST['attachment_ids'] ) ) : [];
	$gallery_data = get_post_meta( $post_id, '_eg_gallery_data', true );
	$in_gallery   = get_post_meta( $post_id, '_eg_in_gallery', true );
...
function envira_gallery_ajax_save_meta() {
	...
	if ( isset( $meta['title'] ) ) {
		$gallery_data['gallery'][ $attach_id ]['title'] = trim( wp_kses( $meta['title'], envira_get_allowed_tags() ) );
	}

	if ( isset( $meta['alt'] ) ) {
		$gallery_data['gallery'][ $attach_id ]['alt'] = trim( esc_attr( $meta['alt'] ) );
	}
...
function envira_gallery_ajax_save_bulk_meta() {
	...
	// Prepare variables.
	$post_id = isset( $_POST['post_id'] ) ? absint( wp_unslash( $_POST['post_id'] ) ) : null;
	// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Sanitized in below loop.
	$image_ids = isset( $_POST['image_ids'] ) ? wp_unslash( $_POST['image_ids'] ) : [];
	// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Sanitized in below loop.
	$meta = isset( $_POST['meta'] ) ? wp_unslash( $_POST['meta'] ) : [];
...
function envira_gallery_ajax_load_gallery_data() {
	...
	if ( ! current_user_can( 'edit_post', $gallery_id ) ) {
		wp_send_json_error( [ 'message' => esc_html__( 'You are not allowed to edit this gallery.', 'envira-gallery-lite' ) ] );
	}
...
function envira_gallery_ajax_activate_addon() {
	...
	// Activate the addon.
	if ( isset( $_POST['plugin'] ) ) {
		$activate = activate_plugin( sanitize_text_field( wp_unslash( $_POST['plugin'] ) ) );
...
function envira_gallery_ajax_dismiss_notice() {
	...
	if ( isset( $_POST['notice'] ) ) {
		...
		$seconds = isset( $_POST['seconds'] ) ? intval( sanitize_text_field( wp_unslash( $_POST['seconds'] ) ) ) : 0;

		$notices->dismiss( $notice, $seconds );
...
function envira_gallery_editor_get_galleries() {
	...
	// Get POSTed fields.
	$search       = isset( $_POST['search'] ) && sanitize_key( $_POST['search'] );
	$search_terms = isset( $_POST['search_terms'] ) ? sanitize_text_field( wp_unslash( $_POST['search_terms'] ) ) : '';
	// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Sanitized in below loop.
	$prepend_ids = isset( $_POST['prepend_ids'] ) ? wp_unslash( $_POST['prepend_ids'] ) : [];
	$results     = [];
	...
	if ( is_array( $prepend_ids ) && count( $prepend_ids ) > 0 ) {
		...
		foreach ( $prepend_ids as $gallery_id ) {
			// Get gallery.
			$gallery = get_post_meta( absint( $gallery_id ), '_eg_gallery_data', true );
...
function envira_gallery_move_media() {
	...
	foreach ( $image_ids as $image_id ) {
		...
		if ( ! is_numeric( $image_id ) || ! isset( $from_gallery['gallery'][ $image_id ] ) ) {
			continue;
		}
...
add_action( 'wp_ajax_envira_dismiss_upsell_notification', 'envira_gallery_hide_upsell_notification_callback' );

/**
 * Helper Method to hide upsell notification.
 *
 * @access public
 * @return void
 */
function envira_gallery_hide_upsell_notification_callback() {
	if ( current_user_can( 'manage_options' ) ) {
		update_option( 'envira_hide_upsell_notification', time() );
	}
	wp_die();
}
